...
`관악정보 → 예술계 소식`·`관악계 소식` 게시판은 하루 한 번 저절로 채워집니다.
...
매일 **오전 9시 23분**, 그리고 **오후 3시 23분**에 한 번 더 돕니다.

정각으로 두면 안 됩니다. GitHub 의 예약 실행은 붐비면 밀리거나 건너뛰는데
정각은 모두가 쓰는 시각이라 가장 붐빕니다. 실제로 `0 0 * * *` 로 두었을 때
하루가 지나도록 한 번도 돌지 않았습니다.

오후 실행은 아침에 건너뛰었을 때를 위한 것입니다. 이미 담은 기사는 다시 담지
않으므로, 아침에 잘 돌았으면 하는 일이 거의 없습니다.
