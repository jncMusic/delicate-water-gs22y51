...
| `NAVER_CLIENT_SECRET` | 그 Client Secret |

키는 <https://developers.naver.com/apps/#/register> 에서 애플리케이션을 등록하고
**사용 API 로 「검색」을 고르면** 발급됩니다.

**주의: 단체 아이디로는 안 됩니다.** 협회 단체 아이디로 등록하면 사용 API
목록에 「검색」이 아예 나오지 않습니다. 그 상태의 키로 호출하면 이렇게 옵니다.
...
개인 아이디로 애플리케이션을 등록해야 「검색」을 고를 수 있습니다. 이 키는
소식을 받아오는 데만 쓰는 개발자 키일 뿐이고, 협회 계정이나 홈페이지 로그인과
아무 관계가 없습니다.
